...
export default function CompletePurchaseDetail({
  ...
  const routerBySectionKey = (key: string) => {
    if (
      ...
      return ROUTES.INSURANCE_MOTORCYCLE.BASIC_DETAIL;
    }
    if (['vehicle_details', 'owner'].includes(key)) {
      return ROUTES.INSURANCE_MOTORCYCLE.PERSONAL_DETAIL;
    }
    if (key === 'policy_plan') {
      return ROUTES.INSURANCE_MOTORCYCLE.PLAN;
    }
    if (['addons', 'driver'].includes(key)) {
      return ROUTES.INSURANCE_MOTORCYCLE.ADD_ON;
    }
    if (['contact_info'].includes(key)) {
      return ROUTES.MOTORCYCLE.REVIEW_INFO_DETAIL;
    }
    return undefined;
  };
  ...
  const handleBackToStepThree = () => {
    router.push(ROUTES.INSURANCE_MOTORCYCLE.ADD_ON);
  };
