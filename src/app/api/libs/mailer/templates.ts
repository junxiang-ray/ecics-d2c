...
export const generateQuoteEmail = ({quote_key='',quote_id='', quote_no='', product_type='Car', name=''} : quoteInfo) => {
  logger.info(`Generating quote email with details: quote_id=${quote_id}, quote_no=${quote_no}, product_type=${product_type}, name=${name}`);

  return `<div style="width: 90% !important;">
      <p>Hi ${name},</p>
      ...
      <table style="border-collapse: collapse; width: 100%;">
        <tr>
          ...
          <td style="border: 1px solid #cccc; text-align: left; padding: 1px 8px;">
            <a href="${process.env.DOMAIN_WEBSITE}/retrieve-quote?key=${quote_key}"
              style="color: #004FFF; text-decoration: underline;">
              ${process.env.DOMAIN_WEBSITE}/retrieve-quote?key=${quote_key}
            </a>
          </td>
        </tr>
      </table>
...
export const generatePersonalInfoEmail = ({name, quote_key}: {name: string,quote_key: string}) => {
  logger.info(`Generating personal info email with details: name=${name}, quote_key=${quote_key}`);
