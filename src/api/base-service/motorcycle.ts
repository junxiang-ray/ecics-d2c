...
export default {
  getMotorcycleQuoteByKey(key: string) {
    return baseClient.get<QuoteResponse>(`${API_GET_QUOTE_BY_KEY}` + key);
  },
  generateMotorcycleQuote(data: QuoteCreationPayload) {
    return baseClient.post<QuoteResponse>('/b2c_motorcycle2/quote', data, {
      headers: {
        'Content-Type': 'application/json',
      },
    });
  },
  saveMotorcycleProposal(data: ProposalPayload, productType: ProductTypeWeb) {
    const url = `/${productType}/proposal`;
    return baseClient.post<any>(url, data);
  },
  saveMotorcycleQuote(key: string, data: any, is_sending_email: boolean) {
    const formatData = {
      data: { ...data },
      key: key,
